...
async def business_signup(business_create_model: BusinessCreateModel):
...
@router.post('/signin')
async def business_signin(form_data: Annotated[OAuth2PasswordRequestForm, Depends()]):
...
async def business(business_schema: Annotated[BusinessSchema, Depends(RouteDeps.get_current_business)]):
    return business_schema.to_mongo()
